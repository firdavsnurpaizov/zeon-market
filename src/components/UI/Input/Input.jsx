    ...
    <div className={style.input}>
      ...
      <div className={style.inputs}>
        ...
        <label className={style.inputUser}>
          <User />
          <input onBlur={e => blurHandler(e)} onChange={e =>nameHandler(e)} value={name} name="name" type="text" placeholder="Как к Вам обращаться?" />
        </label>
          {(phoneDirty && phoneError) && <div style={{color: "red", textAlign: "start"}}>{phoneError} </div> }
        <label className={style.inputUser}>
            <Phone style={{marginRight: 5}} />
          <input onBlur={e => blurHandler(e)} onChange={e => phoneHandler(e)} value={phone} name="phone" type="phone" placeholder="Номер телефона" />
        </label>
      </div>
      <button disabled={!formValid} type="submit" className={style.btn} onClick={orderCall}>
